...
class RecommendPlanChatbotAgentService(
    private val myAppChatModel: ChatLanguageModel,
    ...
    fun chatAi(query: String): UsageDataSizePerMonthResponse {
        val prompt = """
            ...
            분석하여 한 달 사용량에 대한 설명(description)을 하고, 한 달 사용 용량(perMonth)의 결과를 JSON 로 알려줍니다.
            문자와 json 백틱(```json ```) 는 포함하지 말고, 순수 json 형태로만 반환합니다.

            답변은 기본 한글이며, 답변 단위는 'GB' 를 지키세요.
            - description: TEXT 한달동안 사용 예상된 데이터 용량 사이즈.
            - perMonth: LONG GB 단위의 용량
            - calculationDetails: TEXT 계산 상세
        """.trimIndent()
        val messageJsonText = myAppChatModel.chat(prompt).trimIndent()
        val responseJsonText = JsonBackticksRemover.removeBackticks(messageJsonText)
        log.info { responseJsonText }
        return jacksonObjectMapper().readValue(responseJsonText, object : TypeReference<UsageDataSizePerMonthResponse>() {})
    }
    ...
    fun generateResponse(context: String, sizePerMonthly: String, subscription: String): List<RecommendPlanResponse> {
        val prompt = """
            ...
            결과는 무조건 JSON 형식으로 반환하고 리스트형태 입니다. 이때 json 백틱 은 포함하지 않습니다.
            답변은 기본 한글입니다.
        ...
        """.trimIndent()
        val messageJsonText = myAppChatModel.chat(prompt)
        val responseJsonText = JsonBackticksRemover.removeBackticks(messageJsonText)
        log.info { responseJsonText }
        return jacksonObjectMapper().readValue(responseJsonText, object : TypeReference<List<RecommendPlanResponse>>() {})
    }
